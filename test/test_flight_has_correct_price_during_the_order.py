...
class TestFlightHasCorrectPriceDuringTheOrder:
    def test_flight_has_correct_price_during_the_order(self, browser):
        ...
        flight_price_in_popup = your_flight_popup.get_flight_price_from_the_popup()

        logger.info('Click \'Book flight\', get booking information page, and get total flight price')
        flight_price_from_booking_ingo_page = your_flight_popup \
            .click_on_book_flight() \
            .get_flight_price()

        logger.info('Check that all prices are similar during the order process')
